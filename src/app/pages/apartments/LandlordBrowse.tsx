...
export function LandlordBrowse() {
  ...
  useEffect(() => {
    ...
    const loadMarketData = async () => {
      ...
      try {
        const [views, favorites, appUsers, notifications] = await Promise.all([
          fetchApartmentViews(),
          fetchFavorites(),
          fetchUsers(),
  ...
  const SidebarContent = () => (
    <div className="flex h-full flex-col overflow-y-auto">
      ...
      <div className="mt-auto border-t border-white/10 p-4"><button onClick={() => { logout?.(); navigate("/"); }} className="flex w-full items-center gap-3 rounded-lg border border-red-500/30 px-3 py-2.5 text-sm font-bold text-red-400 hover:bg-red-500/10"><LogOut className="h-4 w-4" />Log Out</button></div>
    </div>
  );
  ...
  const PropertyCard = ({ apartment, compact = false }: { apartment: Apartment; compact?: boolean }) => {
    ...
    return (
      <motion.article whileHover={{ y: -3 }} onClick={() => navigate(`/apartment/${apartment.id}`)} className="group cursor-pointer overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-lg">
        ...
        <div className="p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate text-lg font-black text-slate-950">{apartment.title || "Untitled property"}</h3><p className="mt-1 flex items-center gap-1 truncate text-xs font-medium text-slate-500"><MapPin className="h-3.5 w-3.5 shrink-0 text-orange-500" />{location}</p></div><div className="shrink-0 text-right"><p className="text-lg font-black text-orange-600">PHP {Number(apartment.price || 0).toLocaleString("en-PH")}</p><p className="text-[10px] text-slate-500">per month</p></div></div>{landlord?.is_verified && <VerifiedBadge label="Verified Landlord" className="mt-3" />}<div className="mt-4 grid grid-cols-3 gap-2"><div className="rounded-lg bg-emerald-50 p-2.5"><strong className="block text-sm text-emerald-700">{availableRooms}</strong><span className="text-[10px] font-semibold text-slate-500">Available rooms</span></div><div className="rounded-lg bg-blue-50 p-2.5"><strong className="block text-sm text-blue-700">{viewCount}</strong><span className="text-[10px] font-semibold text-slate-500">Views</span></div><div className="rounded-lg bg-rose-50 p-2.5"><strong className="block text-sm text-rose-700">{favoriteCount}</strong><span className="text-[10px] font-semibold text-slate-500">Saved</span></div></div><Button onClick={(event) => { event.stopPropagation(); navigate(`/apartment/${apartment.id}`); }} className="mt-4 h-10 w-full rounded-md bg-orange-500 font-bold text-white hover:bg-orange-600">View Details<ChevronRight className="ml-2 h-4 w-4" /></Button></div>
      </motion.article>
    );
  };
  ...
  const summaryCards = [
    ...
    { label: "Average Rent", value: `PHP ${marketInsights.averageRent.toLocaleString("en-PH")}`, detail: "Monthly rent in La Paz", icon: TrendingUp, tone: "bg-violet-50 text-violet-600" },
